...
public class MudStoveBlockEntity extends SyncedBlockEntity {

    public MudStoveBlockEntity(BlockPos pos, BlockState state) {
        super(PeruBlockEntityTypes.MUD_STOVE.get(), pos, state);
    }
    ...
    public static void cookingTick(Level level, BlockPos pos, BlockState state, MudStoveBlockEntity stove) {
        boolean isStoveLit = (Boolean)state.getValue(MudStoveBlock.LIT);
    ...
    public static void animationTick(Level level, BlockPos pos, BlockState state, MudStoveBlockEntity stove) {
        for(int i = 0; i < stove.inventory.getSlots(); ++i) {
            if (!stove.inventory.getStackInSlot(i).isEmpty() && level.random.nextFloat() < 0.2F) {
                Vec2 stoveItemVector = stove.getStoveItemOffset(i);
                Direction direction = (Direction)state.getValue(MudStoveBlock.FACING);
        ...
        if (this.level != null) {
            for(int i = 0; i < this.inventory.getSlots(); ++i) {
                if (!this.inventory.getStackInSlot(i).isEmpty() && this.level.random.nextFloat() < 0.2F) {
                    ...
                    Direction direction = (Direction)this.getBlockState().getValue(MudStoveBlock.FACING);
                    int directionIndex = direction.get2DDataValue();
                    Vec2 offset = directionIndex % 2 == 0 ? stoveItemVector : new Vec2(stoveItemVector.y, stoveItemVector.x);
